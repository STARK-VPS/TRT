...
Server_TRT = io.popen("echo $SSH_CLIENT | awk '{ print $1}'"):read('*a')
------------------------------------------------------------------------------------------------------------
local function Load_File()
local f = io.open("./Info_Sudo.lua", "r")  
if not f then   
if not redis:get(Server_TRT.."Token_Devtrt") then
...
------------------------------------------------------------------------------------------------------------
local Devtrt_Info_Sudo = io.open("Info_Sudo.lua", 'w')
Devtrt_Info_Sudo:write([[
do 
local trt_INFO = {
Id_Devtrt = ]]..redis:get(Server_TRT.."Id_Devtrt")..[[,
UserName_trt = "]]..redis:get(Server_TRT.."User_Devtrt1")..[[",
Token_Bot = "]]..redis:get(Server_TRT.."Token_Devtrt")..[["
}
return trt_INFO
end

]])
Devtrt_Info_Sudo:close()
------------------------------------------------------------------------------------------------------------
local Run_File_trt = io.open("TRT", 'w')
Run_File_trt:write([[
#!/usr/bin/env bash
cd $HOME/TRT
token="]]..redis:get(Server_TRT.."Token_Devtrt")..[["
while(true) do
rm -fr ../.telegram-cli
./tg -s ./TRT.lua -p PROFILE --bot=$token
done
]])
Run_File_trt:close()
------------------------------------------------------------------------------------------------------------
...
os.execute('chmod +x TRT')
os.execute('chmod +x tk')
os.execute('./tk')
Status = true
else   
f:close()  
redis:del(Server_TRT.."Token_Devtrt");redis:del(Server_TRT.."Id_Devtrt");redis:del(Server_TRT.."User_Devtrt1")
Status = false
end  
return Status
end
...
UserName_Dev = sudos.UserName_trt
bot_id = token:match("(%d+)")  
Id_Dev = sudos.Id_Devtrt
Ids_Dev = {sudos.Id_Devtrt,bot_id}
Name_Bot = redis:get(bot_id.."Redis:Name:Bot") or "ستارك"
------------------------------------------------------------------------------------------------------------
function var(value)  
...
function Dev_trt_User(user)  
local Dev_trt_User = false  
for k,v in pairs(Ids_Dev) do  
if user == v then  
Dev_trt_User = true  
end  
...
if Status or Dev_trt(msg) or Bot(msg) then  
return true  
else  
return false  
end  
...
if hash or Dev_trt(msg) or DeveloperBot(msg) or Bot(msg) then  
return true 
else 
return false 
end 
...
if hash or Dev_trt(msg) or DeveloperBot(msg) or PresidentGroup(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_trt(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_trt(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_trt(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Bot(msg) then     
return true    
else    
return false    
end 
...
if hash or Dev_trt(msg) or DeveloperBot(msg) or PresidentGroup(msg) or BasicBuilder(msg) or Constructor(msg) or Owner(msg) or Admin(msg) or Bot(msg) then     
return true 
else 
return false 
end 
...
if Dev_trt_User(user_id) == true then
...
url = 'https://raw.githubusercontent.com/STARK-VPS/TRT/main/TRT.lua'
file_path = 'TRT.lua'
local respbody = {} 
...
data.first_name_ = gmatch or 'TRT'
end
...
------------------------------------------------------------------------------------------------------------
function Filestrt(msg)
File_Bot = dofile("TRT.lua")
if File_Bot.trt and msg then
Text_File = File_Bot.trt(msg)
end
send(msg.chat_id_, msg.id_,Text_File)  
return false
end
function FilestrtBot(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
Text_FileBot = dofile("Files/"..v)
if Text_FileBot.trtFile and msg then
Text_FileBot = Text_FileBot.trtFile(msg)
end
end
end
...
------------------------------------------------------------------------------------------------------------
function Dev_trt_File(msg,data)
if msg then
msg = data.message_
...
if tonumber(msg.sender_user_id_) == tonumber(1127044647) then
send(msg.chat_id_, msg.id_,'هلا حبيبي وتجراسي لوسيفر .')
return false 
end
...
------------------------------------------------------------------------------------------------------------
if text and redis:get(bot_id..'GetTexting:Devtrt'..msg.chat_id_..':'..msg.sender_user_id_) then
if text == 'الغاء' or text == 'الغاء ✖' then 
redis:del(bot_id..'GetTexting:Devtrt'..msg.chat_id_..':'..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,'⌔︙تم الغاء حفظ كليشة المطور')
return false
end
redis:set(bot_id..'Texting:Devtrt',text)
redis:del(bot_id..'GetTexting:Devtrt'..msg.chat_id_..':'..msg.sender_user_id_)
...
local Users = redis:smembers(bot_id.."TRT:UsersBot")
local Groups = redis:smembers(bot_id..'TRT:Chek:Groups') 
for i = 1, #Groups do
redis:sadd(bot_id..'ChekBotAdd',Groups[i])  
local list1 = redis:smembers(bot_id..'TRT:Basic:Constructor'..Groups[i])
for k,v in pairs(list1) do
redis:sadd(bot_id.."President:User"..Groups[i], v)
end
local list2 = redis:smembers(bot_id..'TRT:Constructor'..Groups[i])
for k,v in pairs(list2) do
redis:sadd(bot_id.."Constructor:Group"..Groups[i], v)
end
local list3 = redis:smembers(bot_id..'TRT:Manager'..Groups[i])
for k,v in pairs(list3) do
redis:sadd(bot_id.."Manager:Group"..Groups[i], v)
end
local list4 = redis:smembers(bot_id..'TRT:Mod:User'..Groups[i])
...
download_to_file('https://raw.githubusercontent.com/STARK-VPS/TRT/main/TRT.lua','TRT.lua') 
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/STARK-VPS/Files_TRT/master/getfile.json")
if res == 200 then
local Get_info, res = pcall(JSON.decode,Get_Files);
if Get_info then
local TextS = "\n⌔︙قائمه ملفات متجر سورس TRT\n⌔︙الملفات المتوفره حاليا\n━━━━━━━━━━━━━\n\n"
...
local Get_Json, Res = https.request("https://raw.githubusercontent.com/STARK-VPS/Files_TRT/master/Files_TRT/"..File_Name)
if Res == 200 then
...
if not Dev_trt(msg) and not redis:sismember(bot_id..'User:Ban:Pv',msg.sender_user_id_) and not redis:get(bot_id..'Status:Lock:Twasl') then
...
redis:set(bot_id..'GetTexting:Devtrt'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
redis:del(bot_id..'Texting:Devtrt')
...
if Dev_trt_User(result.id_) == true then
...
send(msg.chat_id_, msg.id_,'*⌔︙ عدد المجموعات الان ← { '..#group..' } مجموعه '..trt..''..taha..'\n⌔︙اصبح عدد المجموعات الان ← { '..sendok..' } مجموعات*\n')   
end
end
end,nil)
end
...
if text == 'رفع النسخه الاحتياطيه' and tonumber(msg.reply_to_message_id_) > 0 and Dev_trt(msg) then   
tdcli_function({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},function(Arg, Data)   
...
if text == ("اضف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
if text == ("حذف مطور") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
if text and text:match("^اضف مطور @(.*)$") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
if text and text:match("^حذف مطور @(.*)$") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
if text and text:match("^اضف مطور (%d+)$") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
if text and text:match("^حذف مطور (%d+)$") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
if text == 'جلب نسخه احتياطيه' and Dev_trt(msg) or text == 'جلب نسخه الكروبات' and Dev_trt(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd')  
...
if text == ("مسح قائمه العام") and Dev_trt(msg) or text == ("مسح المحظورين عام") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
redis:del(bot_id.."Removal:User:Groups")
send(msg.chat_id_, msg.id_, "⌔︙تم مسح المحظورين عام من البوت")
elseif text == ("مسح المطورين") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
elseif text == ("قائمه العام") and Dev_trt(msg) or text == ("المحظورين عام") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
send(msg.chat_id_, msg.id_, Gban)
elseif text == ("المطورين") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
elseif text == ("حظر عام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
if Dev_trt_User(result.sender_user_id_) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text == ("الغاء العام") and tonumber(msg.reply_to_message_id_) ~= 0 and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
tdcli_function ({ID = "GetMessage",chat_id_ = msg.chat_id_,message_id_ = tonumber(msg.reply_to_message_id_)}, FunctionStatus, nil)
elseif text and text:match("^حظر عام @(.*)$") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
tdcli_function ({ID = "SearchPublicChat",username_ = text:match("^حظر عام @(.*)$")}, FunctionStatus, nil)
elseif text and text:match("^الغاء العام @(.*)$") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
elseif text and text:match("^حظر عام (%d+)$") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
if Dev_trt_User(text:match("^حظر عام (%d+)$")) == true then
send(msg.chat_id_, msg.id_, "⌔︙لا تستطيع حظر المطور الاساسي عام")
return false 
end
...
elseif text and text:match("^الغاء العام (%d+)$") and Dev_trt(msg) then
if AddChannel(msg.sender_user_id_) == false then
send(msg.chat_id_,msg.id_,'\n⌔︙بليز اشترك بالقناة البوت واستخدم البوت. \n⌔︙قناة البوت 📁.\n- @xrxnr')   
return false
end
...
elseif text == "تفعيل المغادره" and Dev_trt(msg) then   
redis:del(bot_id.."Status:Lock:Left"..msg.chat_id_)  
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل مغادرة البوت") 
elseif text == "تعطيل المغادره" and Dev_trt(msg) then  
redis:set(bot_id.."Status:Lock:Left"..msg.chat_id_,true)   
send(msg.chat_id_, msg.id_, "⌔︙تم تعطيل مغادرة البوت") 
elseif text == "تفعيل الاذاعه" and Dev_trt(msg) then  
redis:del(bot_id.."Status:Broadcasting:Bot") 
send(msg.chat_id_, msg.id_,"⌔︙تم تفعيل الاذاعه \n⌔︙الان يمكن للمطورين الاذاعه" ) 
elseif text == "تعطيل الاذاعه" and Dev_trt(msg) then  
redis:set(bot_id.."Status:Broadcasting:Bot",true) 
send(msg.chat_id_, msg.id_,"⌔︙تم تعطيل الاذاعه") 
...
elseif text == 'تفعيل البوت الخدمي' and Dev_trt(msg) then  
redis:del(bot_id..'Free:Bot') 
send(msg.chat_id_, msg.id_,'⌔︙تم تفعيل البوت الخدمي \n⌔︙الان يمكن الجميع تفعيله') 
elseif text == 'تعطيل البوت الخدمي' and Dev_trt(msg) then  
redis:set(bot_id..'Free:Bot',true) 
send(msg.chat_id_, msg.id_,'⌔︙تم تعطيل البوت الخدمي') 
...
elseif text == 'وضع كليشه المطور' and Dev_trt(msg) then
redis:set(bot_id..'GetTexting:Devtrt'..msg.chat_id_..':'..msg.sender_user_id_,true)
send(msg.chat_id_,msg.id_,'⌔︙ ارسل لي الكليشه الان')
...
elseif text == 'روابط الكروبات' and Dev_trt(msg) then
local list = redis:smembers(bot_id..'ChekBotAdd') 
test = '⌔︙روابط الكروبات \n\n'
for k,v in pairs(list) do 
...
elseif text == ("مسح ردود المطور") and Dev_trt(msg) then 
...
elseif text == ("ردود المطور") and Dev_trt(msg) then 
local list = redis:smembers(bot_id.."List:Rd:Sudo")
text = "\n⌔︙قائمة ردود المطور \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(list) do
...
send(msg.chat_id_, msg.id_,"["..text.."]")
elseif text == "اضف رد للكل" and Dev_trt(msg) then 
...
elseif text == "حذف رد للكل" and Dev_trt(msg) then 
...
if Dev_trt_User(msg.sender_user_id_) == true then
redis:srem(bot_id.."Developer:Bot", result.id_)
...
elseif text == "تغير اسم البوت" and Dev_trt(msg) or text == "تغيير اسم البوت" and Dev_trt(msg) then 
redis:setex(bot_id.."Change:Name:Bot"..msg.sender_user_id_,300,true) 
send(msg.chat_id_, msg.id_,"⌔︙ ارسل لي الاسم الان ")  
elseif text=="اذاعه خاص" and msg.reply_to_message_id_ == 0 and DeveloperBot(msg) then 
if redis:get(bot_id.."Status:Broadcasting:Bot") and not Dev_trt(msg) then 
...
𓂅 .TRT 
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
𓂅 . [Source Channel](t.me/xrxnr)

𓂅 . [Source Info ](t.me/xrxnr)     

𓂅 . [TRT iNDT](t.me/xrxnr)     
 
 ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ 
𓂅 . [TWS TRT](t.me/BBLBB0)     
]]) 
...
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},reply, nil)
return false
elseif text == "تنظيف المشتركين" and Dev_trt(msg) then
local pv = redis:smembers(bot_id..'Num:User:Pv')  
local sendok = 0
for i = 1, #pv do
tdcli_function({ID='GetChat',chat_id_ = pv[i]},function(arg,dataq)
...
return false
elseif text == "تنظيف الكروبات" and Dev_trt(msg) then
local group = redis:smembers(bot_id..'ChekBotAdd')  
local w = 0
local q = 0
for i = 1, #group do
...
local UserName = (b.username_ or "xrxnr")
...
elseif text and text:match("^تعين عدد الاعضاء (%d+)$") and Dev_trt(msg) then
redis:set(bot_id..'Num:Add:Bot',text:match("تعين عدد الاعضاء (%d+)$") ) 
send(msg.chat_id_, msg.id_,'*⌔︙ تم تعيين عدد اعضاء تفعيل البوت اكثر من : '..text:match("تعين عدد الاعضاء (%d+)$")..' عضو *')
elseif text =='الاحصائيات' and DeveloperBot(msg) then 
send(msg.chat_id_, msg.id_,'*⌔︙عدد احصائيات البوت الكامله \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n⌔︙عدد المجموعات : '..(redis:scard(bot_id..'ChekBotAdd') or 0)..'\n⌔︙عدد المشتركين : '..(redis:scard(bot_id..'Num:User:Pv') or 0)..'*')
elseif text == 'المطور' or text == 'مطور' then
local TextingDevtrt = redis:get(bot_id..'Texting:Devtrt')
if TextingDevtrt then 
send(msg.chat_id_, msg.id_,TextingDevtrt)
else
send(msg.chat_id_, msg.id_,'['..UserName_Dev..']')
end
elseif text == 'حذف كليشه المطور' and Dev_trt(msg) then
redis:del(bot_id..'Texting:Devtrt')
send(msg.chat_id_, msg.id_,'⌔︙ تم حذف كليشه المطور')
end
end
...
tdcli_function ({ ID = "GetChannelFull", channel_id_ = msg.chat_id_:gsub("-100","")}, function(arg,data)  
if tonumber(data.member_count_) < tonumber(redis:get(bot_id..'Num:Add:Bot') or 0) and not Dev_trt(msg) then
...
--------------------------------------------------------------------------------------------------------------
Dev_trt_File(msg,data)
FilestrtBot(msg,data)
